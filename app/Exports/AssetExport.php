    ...
    public function collection()
    {
        return $this->assets->map(function ($row) {
            return [
                ...
                'บริษัท'      => $row->company->company_name_th ?? '-',
                'แผนก'      => $row->department->department_name_th ?? '-',
                'สาขา'      => $row->branch->branch_name_th ?? '-',
                'ตำแหน่งรับผิดชอบ'      => $row->position->position_name_th ?? '-',
                ...
                'ผู้แจ้งซื้อ'   => $row->presenterUser->full_name ?? '-',
            ];
        });
    }
